...
cor_rb <- as.data.frame(matrix(ncol = 5, nrow = length(unique(rb_na$category)))) # Create a dataframe to save the results
...
for (i in 1:length(unique(rb_na$category))) {
  ...
  cor_rb[i,4] <- shapiro.test(as.numeric(unlist(rb_fake[,2])))$p.value
  cor_rb[i,5] <- shapiro.test(as.numeric(unlist(rb_fake[,7])))$p.value
}
remove(i, j, rb_fake)
...
cor_abd <- as.data.frame(matrix(ncol = 5, nrow = length(unique(rb_na$category)))) # Create a dataframe to save the results
...
for (i in 1:length(unique(rb_na$category))) {
  ...
  cor_abd[i,4] <- shapiro.test(as.numeric(unlist(fake[,2])))$p.value
  cor_abd[i,5] <- shapiro.test(as.numeric(unlist(fake[,6])))$p.value
}
remove(i, j, fake)
