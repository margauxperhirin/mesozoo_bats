...
##### FUNCTION
source("functions/network_and_save_relative.R")
...
for (i in 1:length(unique(m_cn2$membership))) {
  ASV_mod <- both$Representative_Sequence[both$Membership == i]
...
NMar17 <- c('B336','X',0,'N')
RA_other <- rbind(RA_other, NMar17)
...
mod_df[,11:16] <- as.data.frame(lapply(mod_df[,11:16], function(x) if(is.character(x)) as.numeric(x) else x))
...
} # 2 ok for t-test / 14 ok for wilcoxon

wilcoxon <- as.data.frame(matrix(nrow = 13, ncol = 2)) # Easier to do non parametric tests for all of them
...
mod_df$PCoA_Clusters <- clusters$ClustersASV

test_sign <- as.data.frame(matrix(nrow = 13, ncol = 4))
colnames(test_sign) <- c('Variables', 'Shapiro_P', 'Levene_P', 'Test')
...
KW_res <- as.data.frame(matrix(nrow = 13, ncol = 3)) # Easier to do non parametric tests for all of them
...
mod_to_test <- as.data.frame(mod_df[,c('Mod_7','Mod_8','Mod_9','Mod_10','Mod_15','PCoA_Clusters')])

pwc_7 <- dunn_test(data = mod_to_test, formula = Mod_7 ~ PCoA_Clusters, p.adjust.method = "holm") 
pwc_7 <- pwc_7 %>% add_xy_position(x = "PCoA_Clusters")
pwc_8 <- dunn_test(data = mod_to_test, formula = Mod_8 ~ PCoA_Clusters, p.adjust.method = "holm") 
pwc_8 <- pwc_8 %>% add_xy_position(x = "PCoA_Clusters")
pwc_9 <- dunn_test(data = mod_to_test, formula = Mod_9 ~ PCoA_Clusters, p.adjust.method = "holm") 
pwc_9 <- pwc_9 %>% add_xy_position(x = "PCoA_Clusters")
pwc_10 <- dunn_test(data = mod_to_test, formula = Mod_10 ~ PCoA_Clusters, p.adjust.method = "holm") 
pwc_10 <- pwc_10 %>% add_xy_position(x = "PCoA_Clusters")
pwc_15 <- dunn_test(data = mod_to_test, formula = Mod_15 ~ PCoA_Clusters, p.adjust.method = "holm") 
pwc_15 <- pwc_15 %>% add_xy_position(x = "PCoA_Clusters")

bp_Mod_7 <- ggboxplot(mod_df, x = "PCoA_Clusters", y = 'Mod_7') +
  stat_pvalue_manual(pwc_7, hide.ns = TRUE)
bp_Mod_8 <- ggboxplot(mod_df, x = "PCoA_Clusters", y = 'Mod_8') +
  stat_pvalue_manual(pwc_8, hide.ns = TRUE)
bp_Mod_9 <- ggboxplot(mod_df, x = "PCoA_Clusters", y = 'Mod_9') +
  stat_pvalue_manual(pwc_9, hide.ns = TRUE)
bp_Mod_10 <- ggboxplot(mod_df, x = "PCoA_Clusters", y = 'Mod_10') +
  stat_pvalue_manual(pwc_10, hide.ns = TRUE)
bp_Mod_15 <- ggboxplot(mod_df, x = "PCoA_Clusters", y = 'Mod_15') +
