...
community_meta <- left_join(community, metadata, by = 'sampling') # Metadata added
...
pca_var$Sample_time <- community_meta$sample_time # Day/Night 
...
## Do the PCA
morpho_pca <- FactoMineR::PCA(pca_var, quali.sup = 19:22, quanti.sup = 23,
                              row.w = vec_weighted, graph = FALSE)

morpho_pca$eig
# 4 axes significant, 87.73% explained (42.86 - 23.76 - 14.50 - 6.61)
remove(vec_weighted)
...
quanti_morpho <- as.data.frame(morpho_pca$quanti.sup$coord[,1:4])
quanti_morpho <- rownames_to_column(quanti_morpho, 'Variables')

supp_morpho <- rbind(quali_morpho, quanti_morpho)
remove(quali_morpho, quanti_morpho)

## Extract the samplings coordinates for modules study
morphcoord <- supp_morpho[c(32:59),]
metadata$Sampling <- paste0(month(metadata$Month, lab = TRUE, locale = 'English'),
                            sep = '-',
                            metadata$Year, sep = '-',
                            metadata$sample_time) # Cruises dates with Day-Night distinction
metadata$cruisename <- paste0('B', sep = '',
                              metadata$Cruise, sep = '',
                              substr(metadata$sample_time, start = 1, stop = 1))
morphcoord_c <- left_join(morphcoord, metadata[,c(20:21)], by = c('Variables' = 'Sampling'))

write.table(morphcoord_c, file = "img_samplingcoord.csv", sep = ",", quote = FALSE, row.names = F) # Save dataframe

## Add a column for total DW per taxonomic category (from images)
biomass_taxo <- community_meta %>% 
  group_by(category) %>% 
  summarise(DW_taxo = sum(dryweight))

supp_morpho <- left_join(supp_morpho, biomass_taxo, by = c('Variables' = 'category'))
...
supp2 <- supp_morpho[c(1:2,60:62,69:70),c(1:5)]
...
nimg$Type <- c("Other","Crustacea","Other","Mollusca","Other","Copepoda","Gastropoda",
...
colnames(supp_morpho)[7] <- 'Number of images'
colnames(supp_morpho)[8] <- 'Number of images, log10'

supp_morpho$Type <- NA
supp_morpho$Type[3:31] <- c("Other","Crustacea","Other","Mollusca","Other",
                            "Copepoda","Gastropoda","Other","Mollusca","Gelatinous","Crustacea",
                            "Crustacea","Gelatinous","Gastropoda","Copepoda","Crustacea","Gelatinous",
                            "Other","Other","Crustacea","Gastropoda","Gastropoda","Copepoda",
                            "Other","Gelatinous","Gastropoda","Crustacea","Crustacea","Gelatinous")
...
tax12 <- imagesa +
  ...
  geom_text_repel(data = supp_morpho[3:31,],
                  aes(x = Dim.1, y = Dim.2, label = Variables), max.overlaps = Inf,
                  colour = 'black', size = 8) +
...
tax34 <- imagesb +
  geom_point(data = supp_morpho[3:31,],
             ...
             shape = 18, alpha = 0.75) +
  geom_text_repel(data = supp_morpho[3:31,],
                  aes(x = Dim.3, y = Dim.4, label = Variables), max.overlaps = Inf,
                  colour = 'black', size = 8) +
  scale_radius(limits = c(0, NA), range = c(3, 8)) +
  theme_classic(base_size = 30) +
